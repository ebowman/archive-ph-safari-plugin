...
The WebExtension source of truth lives in `extension/` (manifest, background
script, icons). The Xcode wrapper in `app/` was generated by
`xcrun safari-web-extension-converter` and references those resources by
relative path — do not regenerate the project casually, since a converter
